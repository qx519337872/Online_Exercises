...
func CategoryCreate(c *gin.Context) {
	...
	if err != nil {
		log.Panicln("CategoryCreate Error:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "创建分类失败",
		})
...
func CategoryModify(c *gin.Context) {
	...
	if err != nil {
		log.Panicln("CategoryModify Error:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "分类修改失败",
		})
...
func CategoryDelete(c *gin.Context) {
	...
	var cnt int64
	err := models.DB.Model(new(models.ProblemCategory)).Where("category_id= (select id from category_basic where identity=? limit 1)", identity).
		Count(&cnt).Error
	if err != nil {
		log.Panicln("Get ProblemCategory Error:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "获取分类关联的问题失败",
		})
	...
	if err != nil {
		log.Panicln("Delete CategoryBasic Error:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "删除失败",
		})
